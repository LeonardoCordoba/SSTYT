...
# Líneas
lineas_tbl <-unique(rutas$linea)

#c(195,96,8,117,47)

# Para todas las líneas: unique(rutas$linea)
#lineas_tbl <- lineas_tbl[-lin_com]
...
#Configuro el procesamiento en paralelo
registerDoMC(20)

foreach (i=1:length(lineas_tbl)) %dopar% {
  
  linea <- lineas_tbl[i]
  
  if ((linea %in% c(28,45,61, 62, 79, 80, 103, 108, 114, 124, 127, 140, 143, 160, 174, 181, 182, 185, 194))==FALSE) {
    
    count <- count + 1
    lineas_empezadas[count] <- linea
    ...
    for (i in 1:length(tblInternos)){
      ...
      for (j in 1:nrow(tblGPS2)){
        ...
        {trip_direction <- tblGPS2[j, "trip_direction"]}
...
foreach (i=1:length(lineas_tbl)) %dopar% {
  ...
  for (j in 1:length(tblInternos)){
    
    query2 <- paste('select * from tblGPS where interno = ', tblInternos[j],' ORDER BY date_time', sep = "")
    tblGPS2 <- sqldf(query2)
    
    interno <- tblInternos[j]
    ...
    #Para cada servicio
    for (k in tblServicios[-1]) {
      
      #Tomo los datos del servicio
      tblGPS3 <-tblGPS2[tblGPS2$id_servicio==k,]
      ...
      dbGetQuery(con, paste("update gps_dw.",tbl_name,"_",dia, " set trip_direction = ", paste("'",trip_direction_new,"'", sep = ''),
                            ", id_servicio = ", paste("'",k-1,"'", sep='')  ,        " where id_linea = ", paste("'",id_linea,"'", sep = ''), "AND interno = ",interno," AND id >= ", id_inicial," AND id <= ",id_final, sep=''))
      
    }
  }
  lineas_terminadas[i] <- linea
}
